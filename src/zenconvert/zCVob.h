	...
	public:
		...
		*/
		static zCVobData readObjectData(ZenParser& parser)
		{
			...
			parser.getImpl()->readEntry("", &info.pack, sizeof(info.pack), ZenConvert::ParserImpl::ZVT_INT);
			
			if(info.pack)
			{
				packedVobData pd;
				parser.getImpl()->readEntry("", &pd, sizeof(pd), ZenConvert::ParserImpl::ZVT_RAW);
				...
				if(pd.bitfield.hasPresetName)
					parser.getImpl()->readEntry("", &info.presetName, 0, ZenConvert::ParserImpl::ZVT_STRING);

				if(pd.bitfield.hasVobName)
					parser.getImpl()->readEntry("", &info.vobName, 0, ZenConvert::ParserImpl::ZVT_STRING);

				if(pd.bitfield.hasVisualName)
					parser.getImpl()->readEntry("", &info.visual, 0, ZenConvert::ParserImpl::ZVT_STRING);
			...
			else
			{
				...
				parser.getImpl()->readEntry("", info.bbox, sizeof(info.bbox), ZenConvert::ParserImpl::ZVT_RAW_FLOAT);
				parser.getImpl()->readEntry("", &info.rotationMatrix3x3, sizeof(info.rotationMatrix3x3), ZenConvert::ParserImpl::ZVT_RAW);
